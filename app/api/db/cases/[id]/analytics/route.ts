...
export async function GET(req: NextRequest, ctx: RouteContext<"/api/db/cases/[id]/analytics">) {
  try {
...
export async function POST(req: NextRequest, ctx: RouteContext<"/api/db/cases/[id]/analytics">) {
  try {
    const { id: caseId } = await ctx.params;
    const user = await requireUser(req);

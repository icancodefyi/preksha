...
export async function GET(req: NextRequest, ctx: RouteContext<"/api/db/cases/[id]/assignments">) {
  try {
...
export async function POST(req: NextRequest, ctx: RouteContext<"/api/db/cases/[id]/assignments">) {
  try {
    const { id: caseId } = await ctx.params;
    const user = await requireUser(req);

...
export async function GET(req: NextRequest, ctx: RouteContext<"/api/cases/[id]/reports">) {
  try {
...
export async function POST(req: NextRequest, ctx: RouteContext<"/api/cases/[id]/reports">) {
  try {
    const { id: caseId } = await ctx.params;
    const user = await requireUser(req);
